...
export const ViewSiteScreen = (props: any) => {
  const [siteDetails, setSiteDetails] = useState(
    props.route.params.siteDetails,
  );
  ...
  const getCurrentSiteSetting = async () => {
    const siteSetting = await getSiteSettings({
      ...
      userId: props.route.params.user.user_id,
    });
    if (siteSetting.data) {
      ...
      <View>
        <ScrollView>
          <View style={styles.scrollWrapper}>
            ...
            <LabelValueRow
              label="Site Creator"
              value={siteCreator ? siteCreator.adminUserName : ''}
            />
            <LabelValueRow label="Owner Name" value={siteDetails.ownerName} />
            <LabelValueRow
              label="Owner Contract No"
              value={siteDetails.ownerContactNo}
            />
            <LabelValueRow
              label="Address"
              value={siteDetails.siteAddress.AddressLine1}
            />
            <LabelValueRow label="City" value={siteDetails.siteAddress.City} />
            <LabelValueRow
              label="State"
              value={siteDetails.siteAddress.State}
            />
            <LabelValueRow
              label="Pincode"
              value={siteDetails.siteAddress.pincode}
            />
            <LabelValueRow
              label="Start Date"
              value={moment(siteDetails.siteInaugurationDate).format(
                COMMON.DATE_FORMAT,
              )}
            />
            ...
            <LabelValueRow
              label="Deadline"
              value={moment(siteDetails.tentativeDeadline).format(
                COMMON.DATE_FORMAT,
              )}
            />
          </View>
