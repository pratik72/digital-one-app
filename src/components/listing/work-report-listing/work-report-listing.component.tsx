...
export const WorkReportListing = (props: any) => {
  ...
  const allWorkReport = async (page: number = 1) => {
    if (props.currentSite.id) {
      ...
      const allWorkReportRespond = await getAllWorkReport({siteId: props.currentSite.id, page});
      if (allWorkReportRespond && allWorkReportRespond.data[0] && allWorkReportRespond.data[0].data) {
        ...
        const totalPage = Math.ceil(allWorkReportRespond.data[0].count / COMMON.PAGE_SIGE);
  ...
  return (
    <Fragment>
      ...
      {!!totalPages && <View style={styles.paginationView}>
        <PaginationComponent totalPages={totalPages} changePage={allWorkReport} />
      </View>}
    </Fragment>
  );
};
